# views/admin/change_password.py
...
class AdminChangePassword:
    ...
    def build_ui(self):
        ...
        tk.Button(card, text="UPDATE PASSWORD",
                  ...
                  ).grid(row=len(fields)*2, column=0, sticky="ew", pady=(16, 6))

        tk.Button(card, text="⬅  BACK",
                  bg=BTN_GREY, fg=TEXT_WHITE,
                  command=self.go_back, **btn_cfg
                  ).grid(row=len(fields)*2+1, column=0, sticky="ew")
        place_on_canvas(canvas, outer)
        

    def update(self):
        try:
            admin_id = int(self.id_var.get().strip())
        except ValueError:
            messagebox.showerror("Error", "Admin ID must be a number!", parent=self.root)
            return
        user    = self.user_var.get().strip()
        old     = self.old_var.get().strip()
        new     = self.new_var.get().strip()
        confirm = self.confirm_var.get().strip()
        if not all([user, old, new, confirm]):
            messagebox.showerror("Error", "All fields required!", parent=self.root)
            return
        if new != confirm:
            messagebox.showerror("Error", "New passwords do not match!", parent=self.root)
            return
        success = self.model.change_password(admin_id, user, old, new)
        if success:
            messagebox.showinfo("Success", "Password changed successfully!", parent=self.root)
            self.go_back()
        else:
            messagebox.showerror("Error", "Admin ID, Username or Old Password is incorrect!", parent=self.root)
