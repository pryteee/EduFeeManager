...
#         tk.Label(outer, text="ADMIN LOGIN",
...
#                   command=self.login, **btn_cfg
#                   ).grid(row=4, column=0, pady=(16, 6), sticky="ew")

#         tk.Button(card, text="Forgot Password",
#                   bg=BTN_BLUE, fg=TEXT_WHITE,
#                   command=self.open_forgot, **btn_cfg
#                   ).grid(row=5, column=0, pady=4, sticky="ew")

#         tk.Button(card, text="Change Password",
#                   bg=BTN_BLUE, fg=TEXT_WHITE,
#                   command=self.open_change, **btn_cfg
#                   ).grid(row=6, column=0, pady=4, sticky="ew")

#         tk.Button(card, text="⬅  BACK",
#                   bg=BTN_GREY, fg=TEXT_WHITE,
#                   command=self.go_back, **btn_cfg
#                   ).grid(row=7, column=0, pady=(4, 0), sticky="ew")
...
#             return
#         result = self.model.login_admin(user, pwd)
#         if result:
#             messagebox.showinfo("Success", f"Welcome, {result['name']}!", parent=self.root)
#             from UI.admin.manage_counselor import ManageCounselor
#             self.root.withdraw()
#             top = tk.Toplevel(self.root)
#             ManageCounselor(top, self.root, result)
#         else:
#             messagebox.showerror("Error", "Invalid username or password!", parent=self.root)

#     def open_forgot(self):
#         from UI.admin.forgot_password import AdminForgotPassword
#         self.root.withdraw()
#         top = tk.Toplevel(self.root)
#         AdminForgotPassword(top, self.root)

#     def open_change(self):
#         from UI.admin.change_password import AdminChangePassword
#         self.root.withdraw()
#         top = tk.Toplevel(self.root)
#         AdminChangePassword(top, self.root)
...
from models.admin_model import AdminModel
from UI.styles import (
    BG_CARD, BG_INPUT, ACCENT, BTN_BLUE, BTN_GREY,
    TEXT_WHITE, TEXT_MUTED, FONT_TITLE, FONT_SUBTITLE,
    FONT_LABEL, FONT_ENTRY, FONT_BTN, ENTRY_W, PAD_Y,
...
class AdminLogin:
    ...
    def build_ui(self):
        ...
        tk.Button(outer, text="LOGIN",
                  bg=ACCENT, fg=TEXT_WHITE,
                  command=self.login,
                  **btn_cfg).grid(row=6, column=0,
                                  sticky="ew", pady=(16, 6))
        tk.Button(outer, text="Forgot Password",
                  bg=BTN_BLUE, fg=TEXT_WHITE,
                  command=self.open_forgot,
                  **btn_cfg).grid(row=7, column=0,
                                  sticky="ew", pady=4)
        tk.Button(outer, text="Change Password",
                  bg=BTN_BLUE, fg=TEXT_WHITE,
                  command=self.open_change,
                  **btn_cfg).grid(row=8, column=0,
                                  sticky="ew", pady=4)
        tk.Button(outer, text="⬅  BACK",
                  bg=BTN_GREY, fg=TEXT_WHITE,
                  command=self.go_back,
                  **btn_cfg).grid(row=9, column=0,
                                  sticky="ew", pady=(4, 0))
