...
class AdminForgotPassword:
    def __init__(self, root, prev_root):
        ...
        self.admin_found = None
        self.root.title("Admin — Forgot Password")
    ...
    def build_ui(self):
        ...
        self.conf_pass_var = tk.StringVar()
        ...
        # Email or Contact
        tk.Label(card, text="Email OR Contact Number", **lbl_cfg).grid(row=2, column=0, sticky="w", pady=(PAD_Y, 0))
        tk.Entry(card, textvariable=self.ec_var, **entry_cfg).grid(row=3, column=0, sticky="ew", pady=(0, PAD_Y))

        # Validate button
        tk.Button(card, text="VALIDATE IDENTITY",
                  bg=BTN_BLUE, fg=TEXT_WHITE,
                  command=self.validate, **btn_cfg
        ...
        tk.Entry(card, textvariable=self.conf_pass_var, show="*", **entry_cfg).grid(row=9, column=0, sticky="ew", pady=(0, PAD_Y))

        tk.Button(card, text="UPDATE PASSWORD",
                  bg=ACCENT, fg=TEXT_WHITE,
                  command=self.update_password, **btn_cfg
                  ).grid(row=10, column=0, sticky="ew", pady=(12, 6))
    ...
    def validate(self):
        ...
        if not user or not ec:
            ...
            return
        result = self.model.validate_forgot_password(user, ec)
        if result:
            self.admin_found = result
            messagebox.showinfo("Validated", "Identity verified! Now set new password.", parent=self.root)
        else:
            messagebox.showerror("Error", "No match found!", parent=self.root)

    def update_password(self):
        if not self.admin_found:
            messagebox.showerror("Error", "Please validate first!", parent=self.root)
            return
        new  = self.new_pass_var.get().strip()
        conf = self.conf_pass_var.get().strip()
        ...
        if new != conf:
            ...
            return
        success = self.model.update_password_by_username(self.admin_found['username'], new)
        if success:
