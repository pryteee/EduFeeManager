...
class CounselorRegister:
    ...
    def build_ui(self):
        ...
        tk.Label(outer, text="COUNSELOR REGISTRATION",
                 font=FONT_TITLE, bg=BG_DARK, fg=ACCENT).grid(row=0, column=0, pady=(0, 4))
        tk.Label(outer, text="Create your counselor account",
        ...
        tk.Button(card, text="REGISTER", bg=ACCENT, fg=TEXT_WHITE,
                  command=self.register, **btn_cfg
    ...
    def register(self):
        ...
        if self.model.email_exists(email):
            ...
            return
        success, c_id = self.model.register_counselor(name, email, contact, username, password)
        if success:
            messagebox.showinfo("Success",
                                f"Registration Successful!\nYour Counselor ID is: {c_id}",
                                parent=self.root)
            self.go_back()
        else:
