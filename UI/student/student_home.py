...
class StudentHome:
    ...
    def build_ui(self):
        canvas = set_background(
            self.root,
            get_asset_path("student", "dashboard.png")
        )
        outer = tk.Frame(self.root, bg=BG_DARK)
        outer.place(relx=0.5, rely=0.5, anchor="center")

        tk.Label(outer, text="STUDENT PANEL",
    ...
    def open_register(self):
        from UI.student.student_register import StudentRegister
        self.root.withdraw()
        top = tk.Toplevel(self.root)
        StudentRegister(top, self.root)

    def open_login(self):
        from UI.student.student_login import StudentLogin
        self.root.withdraw()
        top = tk.Toplevel(self.root)
        StudentLogin(top, self.root)
